...
public class NetworkManager : MonoBehaviourPunCallbacks
{
   ...
   */
    ...
    {
        //SoundManager.Instance.PlaySeByName("DecisionA");//公開用に再配布不可の音源を削除
        Title.SetActive(false);
    ...
    {
        //SoundManager.Instance.PlaySeByName("DecisionC");//公開用に再配布不可の音源を削除
    ...
    public void ClickLeaveRoom()
    {
        ...
        Debug.Log("部屋から出るよ");
        // ルームから退出する
        PhotonNetwork.LeaveRoom();//PhotonNetwork.IsMessageQueueRunning = true;がないとおかしくなるため注意
    ...
    {
        //SoundManager.Instance.PlaySeByName("DecisionD");//公開用に再配布不可の音源を削除
